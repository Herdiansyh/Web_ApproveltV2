...
export default function Edit({ template }) {
  const { data, setData, put, processing, errors } = useForm({
    ...
    html_view_path: template.html_view_path || '',
    is_active: !!template.is_active,
    version: template.version || 1,
    config_json: template.config_json ? JSON.stringify(template.config_json, null, 2) : '',
  ...
  const submit = (e) => {
    ...
    put(route('templates.update', template.id), { data: payload });
  };

  return (
    <AuthenticatedLayout header={<h2 className="text-xl font-semibold">Edit Template</h2>}>
      ...
      <form onSubmit={submit} className="p-6 space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <textarea className="mt-1 w-full border rounded px-3 py-2 font-mono text-xs min-h-[160px]" value={data.config_json} onChange={(e) => setData('config_json', e.target.value)} />
