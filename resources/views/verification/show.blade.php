...
<body>
    <div class="container">
        ...
        @if (!$isValid)
            <p style="margin-top:16px;">Token verifikasi tidak ditemukan atau tidak sesuai. Silakan hubungi penerbit
                dokumen.</p>
        ...
        @else
            <div class="grid">
                ...
                <div class="value">{{ $submission->user?->name ?? '-' }}
                    ({{ $submission->user?->division?->name ?? '-' }})</div>

                <div class="label">Dibuat pada</div>
                <div class="value">{{ $submission->created_at?->format('d M Y H:i') }}</div>
            </div>

            @if (!empty($submission->qr_code_path) || !empty($qrSvg))
                <div style="margin-top:16px;" class="muted">QR Code penyusun dokumen:</div>

                @if (!empty($qrSvg))
                    <div class="qr-inline" style="width:140px; height:140px; border:1px solid #e5e7eb; border-radius:8px; overflow:hidden;">
                        {!! $qrSvg !!}
                    </div>
                @else
                    @php
                    ...
                    @else
                        <img alt="QR Code" src="{{ asset('storage/' . $submission->qr_code_path) }}"
                            style="width:140px; height:140px; border:1px solid #e5e7eb; border-radius:8px;" />
                    @endif
                @endif
            @endif

            <a class="btn" href="{{ route('submissions.printDocument', $submission->id) }}" target="_blank">Lihat
                Halaman Cetak</a>
